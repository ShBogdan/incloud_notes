...
//        textInputLayout = findViewById(R.id.textMailInput)
...
//        loginButton = findViewById(R.id.loginButton)
        ...
        if(sharedPreferencesManager.getUser()!=null){
            binding.emailInput.text = SpannableStringBuilder(user?.email)
            binding.passwordInput.text = SpannableStringBuilder(user?.password)
        }
        ...
        binding.emailInput.setOnFocusChangeListener { _, hasFocus ->
            if (!hasFocus) {
                ...
                if (!isValidEmail(email)) {
                    binding.textMailInput.error = getString(R.string.email_hint)
                } else {
                    binding.textMailInput.error = null
                }
            }
        }
    }
    ...
    private fun extractEmail(): String {
        return binding.emailInput.text?.toString()?.trim() ?: ""
    }
    ...
    private fun extractPassword(): String {
        return binding.passwordInput.text?.toString()?.trim() ?: ""
    }
